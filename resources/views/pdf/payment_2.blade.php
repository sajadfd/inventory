...
<style>
   ...
   <table>
        ...
        <tr>
           <td class="title" colspan="3">الديون الأخرى</td>
           <td class="body" colspan="3">{{$payment->bill->getPersonDebtsAttribute() - ($payment->bill->getRemainingPriceAttribute() - $payment->bill->getPayedPriceAttribute())}} {{__($payment->currency)}}</td>
        </tr>

        <tr>
           <td class="title" colspan="1">كود الفاتورة </td>
           <td class="body bill" colspan="2"><img src="data:image/svg+xml;base64,{{base64_encode($qrCodeSvg_bill)}}" alt="{{$payment->bill->code}}"></td>

           <td class="title" colspan="1">كود السند</td>
